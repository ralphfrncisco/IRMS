...
import { Funnel, Eye, Loader2, Search, X } from 'lucide-react'; // ✅ added Search, X
...
export default function TableSection() {
    ...
    const filteredOrders = useMemo(() => {
        ...
        // Search filter — matches order ID, customer, purchased items, balance, date, status
        if (searchQuery.trim()) {
            ...
            filtered = filtered.filter(order =>
                ...
                formatCurrency(order.remaining_balance)?.toLowerCase().includes(q) ||
        ...
        return filtered;
    }, [dateRangeFilter, customerFilter, paymentStatusFilter, salesData, searchQuery]); // ✅ added searchQuery
    ...
    return (
        <div className="rounded-2xl border bg-white border-slate-200 dark:bg-[#111] dark:border-white/10 transition-all duration-300 mb-25 pb-5 md:pb-0">
            <div className="p-4 border-b border-slate-100 dark:border-white/10 gap-4 w-full md:w-auto space-y-2">
                <div className="flex items-center justify-between w-full py-2">
                    ...
                    <div className="flex items-center gap-2 relative" ref={filterRef}>
                        ...
                        {showFilters && (
                            <div className="absolute top-full right-0 mt-2 w-55 p-4 bg-white dark:bg-[#111] border border-slate-200 dark:border-white/10 rounded-xl shadow-xl z-50 space-y-3 animate-in fade-in zoom-in duration-200">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-white/10">
                        {loading ? (
                            ...
                            filteredOrders.map((order) => (
                                <tr key={order.db_id} className="text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group">
                                    ...
                                    {visibleColumns['AMOUNT'] && (
                                        <td className="p-4 text-center text-sm font-semibold text-red-500">
