...
function AddSalaryModal({ isOpen, onClose }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex py-2 items-center justify-center overflow-y-auto">
            <div 
                className="flex flex-col h-auto md:max-h-[80vh] bg-white dark:bg-[#111] p-4 md:p-6 rounded-2xl shadow-2xl w-full max-w-md mx-2 border border-slate-200 dark:border-white/10" 
                ...
                <form onSubmit={handleFormSubmit} id = "saveSalaryForm" className="flex-grow overflow-y-auto space-y-9 md:pr-2">
                    <div className="grid grid-cols-1 gap-6 max-w-85 md:max-w-full">
                        <div className="relative w-full">
                            <label className="block text-sm font-semibold text-slate-700 dark:text-white/70 mb-1">Employee's Name</label>
                            <input type="text" name="employeeName" value={formValues.employeeName} onChange={handleInputChange} className="w-full text-slate-700 dark:text-slate-200 px-3 py-1.5 h-10 rounded-lg border border-slate-300 dark:border-white/5 dark:bg-[#1E1E1E] focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all" />
                        </div>
                        <div className="relative w-full">
                            ...
                            <div className="relative">
                                ...
                                <input type="text" name="amount" value={formValues.amount} onChange={handleInputChange} placeholder="0.00" autoComplete="off" className="w-full text-slate-700 dark:text-slate-200 pl-9 pr-3 py-1.5 h-10 rounded-lg border border-slate-300 dark:border-white/5 dark:bg-[#1E1E1E] focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all" />
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="relative h-10 w-full group">
                                <div className="absolute inset-0 flex items-center justify-between px-3 rounded-lg border border-slate-300 dark:border-white/5 bg-white dark:bg-[#1E1E1E] text-sm text-slate-700 dark:text-slate-200 overflow-hidden">
                                    <span className={`truncate mr-2 ${formValues.transactionDate ? "text-slate-700 dark:text-white/50" : "text-slate-400 dark:text-white/50"}`}>
                                        {formValues.transactionDate ? formatDateDisplay(formValues.transactionDate) : "Select Date"}
                                    </span>
                                    <div className="flex items-center gap-2 flex-shrink-0">
                                        <button type="button" onClick={(e) => { e.stopPropagation(); setToday(); }} className="relative z-20 px-2 py-1 text-[10px] font-black uppercase bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-white/50 rounded hover:bg-blue-600 hover:text-white transition-all shadow-sm whitespace-nowrap">Today</button>
